import numpy as np
from Cell import *
from Settings import *
import pygame
class Board:
    def __init__(self):
        self.board_surface = pygame.Surface((WIDTH, HEIGHT))

        self.number_of_mines = AMOUNT_MINES
        self.create_grid()

    def create_user_grid(self):
        self.user_grid = [[Cell(col, row, 'N', tile_unknown) for col in range(COLS)] for row in range(ROWS)]

    def create_grid(self):
        self.grid = [[Cell(col, row, 0, tile_empty) for col in range(COLS)] for row in range(ROWS)]

    def set_mines(self, firstChosen): 
        mines_set = 0
        while mines_set < self.number_of_mines:
            while True:
                random_column = np.random.randint(0, 9)
                random_row = np.random.randint(0, 9)
                if (
                    self.grid[random_row][ random_column].cell_state != -1
                    and (random_row, random_column) != firstChosen
                ):
                    self.grid[random_row][random_column].cell_state = -1
                    self.grid[random_row][random_column].image = tile_mine
                    break


            # adjusting the hints
            if random_row != 0:
                i = random_row - 1
                r = -1
            else:
                i = random_row
                r = 0

            if random_column != 0:
                j = random_column - 1
                l = -1
            else:
                j = random_column
                l = 0

            row_neighbours = 2 if r == 0 else 3
            column_neighbours = 2 if l == 0 else 3

            row_neighbours = min(row_neighbours, ROWS - i)
            column_neighbours = min(column_neighbours, COLS -j )

            for k in range(row_neighbours):              
                for m in range(column_neighbours):
                    if self.grid[i][j].cell_state != -1 and i < COLS:
                        self.grid[i][j].cell_state += 1
                        a=self.grid[i][j].cell_state
                        self.grid[i][j].image=tile_numbers[a-1]
                    j += 1
                j = random_column + l
                i += 1
            mines_set += 1

    def draw(self, screen):
        for row in self.user_grid:
            for cell in row:
                cell.draw(self.board_surface)
        screen.blit(self.board_surface,(0,0))

    def show_grid(self, grid):
        for row in grid:
            temp=[col.cell_state for col in row]
            print(temp)